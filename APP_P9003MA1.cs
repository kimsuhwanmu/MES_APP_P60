...
{
    public partial class APP_P9003MA1 : UserControl
    {
        ...
        private void btn_Before_ButtonClick(object sender, EventArgs e)
        {
            ...
            //        if (vFirstDisplayedScrollingRowIndex < 0)
            //            Grid3.FirstDisplayedScrollingRowIndex = 0;
            //        else
            //            Grid3.FirstDisplayedScrollingRowIndex = Grid3.FirstDisplayedScrollingRowIndex - vDisplayedRowCount;
            //    }
            //}
            ...
            //        if (vFirstDisplayedScrollingRowIndex < 0)
            //            Grid4.FirstDisplayedScrollingRowIndex = 0;
            //        else
            //            Grid4.FirstDisplayedScrollingRowIndex = Grid4.FirstDisplayedScrollingRowIndex - vDisplayedRowCount;
            //    }
            //}
            else
            {
        ...
        private void btn_Next_ButtonClick(object sender, EventArgs e)
        {
            ...
            //            Grid3.FirstDisplayedScrollingRowIndex = Grid3.Rows.Count - 1;
            //        else
            //            Grid3.FirstDisplayedScrollingRowIndex = vFirstDisplayedScrollingRowIndex;
            //    }
            //}
            //else if (sGrid_Current_id == 4)    //그리드 2가 활성화 상태이면 다음 버튼을 누르면 그리드2의 페이지가 변경됨
            //{
            ...
            //            Grid4.FirstDisplayedScrollingRowIndex = Grid4.Rows.Count - 1;
            //        else
            //            Grid4.FirstDisplayedScrollingRowIndex = vFirstDisplayedScrollingRowIndex;
            //    }
            //}
            else
            {
        ...
        private void btn_Bad_Qty_Insert_ButtonClick(object sender, EventArgs e)
        {
            ...
            if (Grid2.CurrentRow == null)
            {
                TGSControl.UsrFunction.MessageBoxErr("불량등록할 LOT를선택하세요.", "불량등록");
                return;
            }
            ...
            if (Convert.ToDecimal(Grid2.CurrentRow.Cells[grid2_COLUMN.GOOD_QTY].Value.ToString()) == 0)
            {
                ...
                TGSControl.UsrFunction.MessageBoxErr("양품LOT 실적을 선택하세요.", "불량등록");
                return;
            }

            POP_P9003PA2 myPopupForm = new POP_P9003PA2();
            ...
            try
            {
                ...
                dt.Rows[0]["RESULT_NO"] = Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_NO].Value.ToString();
                dt.Rows[0]["RESULT_SEQ"] = Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_SEQ].Value.ToString();
                dt.Rows[0]["WK_ORD_NO"] = Grid1.CurrentRow.Cells[grid1_COLUMN.WK_ORD_NO].Value.ToString();

                dt.Rows[0]["WORKER_ID"] = Grid2.CurrentRow.Cells[grid2_COLUMN.WORKER_ID].Value.ToString();
                dt.Rows[0]["LOT_NO"] = Grid2.CurrentRow.Cells[grid2_COLUMN.LOT_NO].Value.ToString();
                ...
                dt.Rows[0]["PROD_QTY"] = Grid2.CurrentRow.Cells[grid2_COLUMN.PROD_QTY].Value.ToString();
                dt.Rows[0]["GOOD_QTY"] = Grid2.CurrentRow.Cells[grid2_COLUMN.GOOD_QTY].Value.ToString();
                dt.Rows[0]["BAD_QTY"] = Grid2.CurrentRow.Cells[grid2_COLUMN.BAD_QTY].Value.ToString();
                ...
                if (dResult == DialogResult.OK)
                {
                    ...
                    //실적처리 완료
                    string vRESULT_NO = Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_NO].Value.ToString();
                    string vRESULT_SEQ = Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_SEQ].Value.ToString();


                    if (LabelPrint_Work_Lot_RetriveData(sPLANT_CD, vRESULT_NO, vRESULT_SEQ, true) == true)    //바코드 출력
                    {
                    ...
                    {
                        if (Grid1.Rows[i].Cells[grid1_COLUMN.RESULT_NO].Value.ToString() == vRESULT_NO)
                        {
                            Grid1.Rows[i].Cells[grid1_COLUMN.ITEM_CD].Selected = true;

                            DBQuery_lot_no(vRESULT_NO); //상세내역을 다시조회한다.
                            break;
                        }
            ...
            {
                TGSControl.UsrFunction.MessageBoxErr(se.Message, "LABEL 프린터 에러");
            }
            finally
            {
                myPopupForm.Dispose();
        ...
        private void btn_Label_Print_ButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                bool vPrintFlag = false;

                for (int i = 0; i < Grid2.Rows.Count; i++)
                {
                    System.Drawing.Image imgCheck = (System.Drawing.Image)Grid2.Rows[i].Cells[grid2_COLUMN.CHECKFLAG].Value;

                    if (imgCheck.Tag.ToString() == "Y")
                    {
                        string vRESULT_NO = Grid2.Rows[i].Cells[grid2_COLUMN.RESULT_NO].Value.ToString();
                        string vRESULT_SEQ = Grid2.Rows[i].Cells[grid2_COLUMN.RESULT_SEQ].Value.ToString();

                        if (LabelPrint_Work_Lot_RetriveData(sPLANT_CD, vRESULT_NO, vRESULT_SEQ, false) == true)     //바코드 출력
                        {
                            vPrintFlag = true;
                        }
        ...
        private void btn_RESULT_CANCEL_ButtonClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //if (vFlag)
                //{
                //    if (TGSControl.UsrFunction.MessageBoxYesNo("체크 선택된 실적을 취소하시겠습니까?", "실적취소") == DialogResult.Yes)
                //    {
                //        bool vSaveFlag = false;
                //        int vResultOKCount = 0;
                ...
                //            if (imgCheck.Tag.ToString() == "Y")
                //            {
                //                if (DBDelete_Work_Result_Cancel(Grid2.Rows[i].Cells[grid2_COLUMN.RESULT_NO].Value.ToString(), Grid2.Rows[i].Cells[grid2_COLUMN.RESULT_SEQ].Value.ToString()))
                //                {
                //                    imgCheck = Properties.Resources.CheckBoxOK;
                //                    imgCheck.Tag = "O";
                //                    Grid2.Rows[i].Cells[grid2_COLUMN.CHECKFLAG].Value = imgCheck;
                //                    Grid2.Rows[i].Visible = false;
                //                    vResultOKCount = vResultOKCount + 1;

                //                    vSaveFlag = true;
                //                }
                //                else
                //                {
                //                    imgCheck = Properties.Resources.CheckBoxFalse;
                //                    imgCheck.Tag = "F";
                //                    Grid2.Rows[i].Cells[grid2_COLUMN.CHECKFLAG].Value = imgCheck;
                ...
                //            TGSControl.UsrFunction.MessageBoxInfo("실적 취소처리 되었습니다.", "취소완료");
                //            //삭제 성공이면 내역 다시조회
                //            if (Grid2.Rows.Count == vResultOKCount) //삭제한 내역이 그리드2 전체이면 전체를 다시조회함
                //            {
                //                DBQuery();
                //            }
                //            else
                //            {
                //                //일부만 삭제되었으면 하단만 다시 조회
                //                DBQuery_lot_no(Grid1.CurrentRow.Cells[grid1_COLUMN.RESULT_NO].Value.ToString());
                //            }
                ...
                //            if (vResultOKCount > 0)
                //            {
                //                TGSControl.UsrFunction.MessageBoxInfo("일부 품목에서 취소 오류가 있습니다. 확인 바랍니다.", "취소오류");

                //                for (int i = Grid2.Rows.Count - 1; i >= 0; i--)
                //                {
                //                    System.Drawing.Image imgCheck = (System.Drawing.Image)Grid2.Rows[i].Cells[grid2_COLUMN.CHECKFLAG].Value;

                //                    if (imgCheck.Tag.ToString() == "O") //저장완료된 데이타 삭제
                //                    {
                ...
                //            else
                //            {
                //                TGSControl.UsrFunction.MessageBoxInfo("실적 취소 오류가 있습니다. 확인 바랍니다.", "취소오류");
                //            }
                ...
                //{
                    ...
                    //체크 선택된 것이 없으면 current Row를 삭제한다.

                    if (Grid2.CurrentRow == null)
                    {
                        TGSControl.UsrFunction.MessageBoxInfo("실적 취소할 LOT를선택하세요.", "실적취소");
                        return;
                    }



                    if (TGSControl.UsrFunction.MessageBoxYesNo("실적 취소하시겠습니까?", "실적취소") == DialogResult.Yes)
                    {

                        if (DBDelete_Work_Result_Cancel(Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_NO].Value.ToString(), Grid2.CurrentRow.Cells[grid2_COLUMN.RESULT_SEQ].Value.ToString()))
                        {
                            TGSControl.UsrFunction.MessageBoxInfo("실적 취소처리 되었습니다.", "취소완료");
                            ...
                            {
                                string vResult_no = Grid1.CurrentRow.Cells[grid1_COLUMN.RESULT_NO].Value.ToString();
