...
{
    public partial class APP_P9005MA2 : UserControl
    {
        #region ▶ 1. Declaration part  //선언부

        String _PG_NM = "PLC DATA 조회";
        ...
        {
            ///<summary>설비</summary>
            public const string RESOURCE_CD = "RESOURCE_CD";
        ...
        private void InitSpreadSheet(int p_GridIndex)   //선택된 스프레드시트를 초기화 한다.
        {
            if (p_GridIndex == 1)
            {
                ...
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.RESOURCE_CD, "설비", 120));
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.INSRT_DT, "등록시각", 100));
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.WC_CD, "작업장", 120));
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.STATUS, "상태", 100)); // 콤보박스 형태로 변경예정
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.PRODT_ORDER_NO, "제조오더", 160));
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.PROD_QTY, "생산량", 80));
                Grid1.Columns.Add(SetColumnEdit(grid1_COLUMN.BAD_QTY, "불량", 80));
                #endregion

                #region ■■ 3.1.2 Formatting grid information

                Grid1.Columns[grid1_COLUMN.RESOURCE_CD].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                Grid1.Columns[grid1_COLUMN.INSRT_DT].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grid1.Columns[grid1_COLUMN.WC_CD].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grid1.Columns[grid1_COLUMN.STATUS].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        private bool FncGet_list(string @FLAG)
        {
            ...
            string strData = "";
            strData += "EXEC XUSP_MES_APP_P9005MA2_GET1 ";
            ...
            try
            {
                ...
                if (dataT.Rows.Count > 0)
                {
                    for (int i = 0; i < dataT.Rows.Count; i++)
                    {
                        ...
                        Grid1.Rows[i].Cells[grid1_COLUMN.RESOURCE_CD].Value = dataT.Rows[i][grid1_COLUMN.RESOURCE_CD].ToString();
                        Grid1.Rows[i].Cells[grid1_COLUMN.INSRT_DT].Value = Convert.ToDateTime(dataT.Rows[i][grid1_COLUMN.INSRT_DT]);
                        Grid1.Rows[i].Cells[grid1_COLUMN.WC_CD].Value = dataT.Rows[i][grid1_COLUMN.WC_CD].ToString();
                        Grid1.Rows[i].Cells[grid1_COLUMN.STATUS].Value = dataT.Rows[i][grid1_COLUMN.STATUS].ToString();
                        Grid1.Rows[i].Cells[grid1_COLUMN.PRODT_ORDER_NO].Value = dataT.Rows[i][grid1_COLUMN.PRODT_ORDER_NO].ToString();
                        Grid1.Rows[i].Cells[grid1_COLUMN.PROD_QTY].Value = Convert.ToDecimal(dataT.Rows[i][grid1_COLUMN.PROD_QTY]);
                        Grid1.Rows[i].Cells[grid1_COLUMN.BAD_QTY].Value = Convert.ToDecimal(dataT.Rows[i][grid1_COLUMN.BAD_QTY]);
                    }
                }
            }
            catch (Exception ex)
